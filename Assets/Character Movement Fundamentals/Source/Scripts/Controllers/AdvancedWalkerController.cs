...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller {
        ...
        [SerializeField]
        private CharacterKeyboardInput _keyInput;
        //Jump key variables;
		...
		void ControllerUpdate()
		{
			...
			if (!PlayerController.instance.isDead)
			{
				...
				{
					if (_keyInput._joystikMove.Vertical > 0)
					{
						PlayerController.instance.SetPlayerAnimationState("walkF");
					}
					else if (_keyInput._joystikMove.Vertical < 0)
					{
						PlayerController.instance.SetPlayerAnimationState("walkB");
					}
				}
				else
				{
					PlayerController.instance.SetPlayerAnimationState("idle");
				}
			}
